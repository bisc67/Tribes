...
void        voicestream_PeriodicUpdate(void)
{
    ...
    while (1)
    {
        ...
        pRequest = (stream_request *)g_pcAudio.m_pqPendingStreamRequests->Recv(MQ_BLOCK);
        ASSERT(pRequest);
        ...
        if (pRequest->m_FileId < 0)
        {
            ...
            if (!status)
            {
                ...
                x_memset(pRequest->m_pData,0,pRequest->m_Length);
                x_DebugMsg("CD Read error occurred at lsn %d\n",lsn);
            }
        ...
        pRequest->m_pqReply->Send(pRequest,MQ_BLOCK);
    }
}
...
//    return;
    ...
    pStreams = (voice_stream*)x_malloc(nBuffers * sizeof(voice_stream) );
    ASSERT(pStreams);
// Create our stream list
    g_pcAudio.m_pqFreeStreams = new xmesgq(nBuffers);
    
    x_memset(pStreams,0,sizeof(voice_stream)*nBuffers);
...
// Message queue for completed pre-read blocks
        pStreams->m_pqReadyData = new xmesgq(AUD_STREAM_READAHEAD);
        g_pcAudio.m_pqFreeStreams->Send(pStreams,MQ_BLOCK);
        pStreams++;
    }
    g_pcAudio.m_pStreamBuffers =(u8*)x_malloc(AUD_STREAM_BUFFER_SIZE * AUD_MAX_STREAMS * AUD_STREAM_READAHEAD);
    ASSERT(g_pcAudio.m_pStreamBuffers);
// Message queue for the currently available buffers
    g_pcAudio.m_pqAvailableStreamRequests = new xmesgq(AUD_MAX_STREAMS * AUD_STREAM_READAHEAD);
// Message queue for the currently outstanding read requests
    g_pcAudio.m_pqPendingStreamRequests = new xmesgq(AUD_MAX_STREAMS * AUD_STREAM_READAHEAD);
    for (i=0;i<AUD_MAX_STREAMS * AUD_STREAM_READAHEAD;i++)
    {
        g_pcAudio.m_StreamRequests[i].m_pData = &g_pcAudio.m_pStreamBuffers[i*AUD_STREAM_BUFFER_SIZE];
        g_pcAudio.m_pqAvailableStreamRequests->Send(&g_pcAudio.m_StreamRequests[i],MQ_BLOCK);
    }
    g_pcAudio.m_pStreamThread = new xthread(voicestream_PeriodicUpdate,"VoiceStream periodic Update",8192,2);
...
#endif
    pStream = (voice_stream*)g_pcAudio.m_pqFreeStreams->Recv(MQ_NOBLOCK);
//    pStream = (voice_stream*)x_malloc(sizeof(voice_stream));
    if (!pStream) 
        return NULL;
...
//    voicestream_UpdateReadahead(pStream);
    ...
    s32 Loop = 1;
    if (pElement->m_Flags & AUDFLAG_HARDWARE_LOOP)
    {
			Loop = INFINITE_LOOP;
    }
    else if (pElement->m_Flags & AUDFLAG_LOOPED)
    {
			Loop = INFINITE_LOOP;
    }

    // The loop flags isn't being implemented yet.
    pStream->m_SpuBuffer = g_Miles.LoadStream(pElement->m_pAttributes->m_Name, Loop);
    
    // The stream wan't able to initialized.
    if (pStream->m_SpuBuffer == -1)
    {
        x_free(pStream);
        return NULL;
    }

    g_Miles.PlayStream(pStream->m_SpuBuffer);
    g_Miles.SetStreamPriority(pStream->m_SpuBuffer, pElement->m_pAttributes->m_Priority);
    g_Miles.SetStreamVolume(pStream->m_SpuBuffer, pElement->m_pOwner->m_State.m_Volume);
    pStream->m_pOwner = pElement;
    return (pStream);
}
...
//    {
//        pReq=(stream_request*)pStream->m_pqReadyData->Recv(MQ_NOBLOCK);
//        if (pReq)
//        {
//            status = g_pcAudio.m_pqAvailableStreamRequests->Send(pReq,MQ_NOBLOCK);
//            ASSERT(status);
...
//    }
*/
    g_Miles.UnloadStream(pStream->m_SpuBuffer);

    pStream->m_Status = STREAMSTAT_IDLE;
    g_pcAudio.m_pqFreeStreams->Send(pStream,MQ_BLOCK);
//    x_free(pStream);
...
{
    // Set the volume.
    g_Miles.SetStreamVolume(pStream->m_SpuBuffer, pStream->m_pOwner->m_pOwner->m_State.m_Volume);

    // This better never happen.
    if (pStream->m_SpuBuffer == -1)
        ASSERT(FALSE);

    // If the stream is done playing, let is go.
    if (g_Miles.GetStreamStatus(pStream->m_SpuBuffer) == SMP_DONE)
    {        
        return TRUE;
    }
...
xbool voicestream_UpdateReadahead(voice_stream *pStream)
{
         ...
         (pStream->m_ReadaheadRemaining < (pStream->m_BufferCount/2) ))
    {
        ...
        {
            pRequest = (stream_request *)g_pcAudio.m_pqAvailableStreamRequests->Recv(MQ_NOBLOCK);
            ...
            pRequest->m_pqReply = pStream->m_pqReadyData;
            g_pcAudio.m_pqPendingStreamRequests->Send(pRequest,MQ_BLOCK);
            pStream->m_ReadaheadInProgress++;
...
xbool voicestream_UpdateHybridBuffer(voice_stream *pStream,s32 BufferOffset,s32 remaining)
{
    ...
    pData = (byte*)pStream->m_pOwner->m_pHybridBuffer + pStream->m_ReadaheadPosition;
    pStream->m_ReadaheadPosition+=AUD_STREAM_BUFFER_SIZE;
...
//        spu_InitVoice(pStream->m_pVoice,pStream->m_SpuBuffer,pStream->m_pVoice->m_pOwner->m_pAttributes->m_ADSR1,pStream->m_pVoice->m_pOwner->m_pAttributes->m_ADSR2);
        spu_SetKey(pStream->m_pVoice,1);
...
xbool voicestream_UpdateBuffer(voice_stream *pStream,s32 BufferOffset,s32 remaining)
{
    ...
    //
    //
    pRequest = (stream_request*)pStream->m_pqReadyData->Recv(MQ_NOBLOCK);
    if (!pRequest)
    {
#ifdef X_DEBUG
//        printf("Block request delayed, offset=%d, remain=%d\n",pStream->m_PlayRemaining,remaining);
    ...
    if (Stereo)
    {
        ...
        pSiblingRequest = (stream_request*)pStream->m_pqReadyData->Recv(MQ_NOBLOCK);
        ...
        {
#ifdef X_DEBUG
            x_DebugMsg("Sibling block request delayed, offset=%d, remain=%d\n",pStream->m_PlayRemaining,remaining);
#endif
            status = pStream->m_pqReadyData->Send(pRequest,MQ_NOBLOCK|MQ_JAM);
            ASSERT(status);
            return FALSE;
        }
        ...
        pStream->m_PlayRemaining--;
        pStream->m_ReadaheadInProgress--;
        g_pcAudio.m_pqAvailableStreamRequests->Send(pSiblingRequest,MQ_BLOCK);
    }
...
//            spu_InitVoice(pStream->m_pSibling->m_pVoice->m_Id,pStream->m_pSibling->m_SpuBuffer,pStream->m_pOwner->m_pAttributes->m_ADSR1,pStream->m_pOwner->m_pAttributes->m_ADSR2);
            spu_SetKey(pStream->m_pSibling->m_pVoice,1);
        }
        pStream->m_Status = STREAMSTAT_PLAYING;
    }
    status = g_pcAudio.m_pqAvailableStreamRequests->Send(pRequest,MQ_NOBLOCK);
    ASSERT(status);
    return TRUE;
}
