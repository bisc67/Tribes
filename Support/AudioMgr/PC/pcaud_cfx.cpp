...
cfx_pool *cfxpool_Expand(s32 nEntries)
{
    ...
    {
       g_pcAudio.m_pCfxPool = pPool;
       pPool->m_pNext = NULL;
    }

    for (i=0;i<nEntries;i++)
    {
        cfx_Free(&g_pcAudio.m_pCfxPool->m_CfxElements[i]);
    }
    return pPool;
}
...
void cfx_Init(void)
{
    ...
    allocamount = g_pcAudio.m_ChannelCount * sizeof(cfx) + sizeof(cfx_pool);
    
    g_pcAudio.m_pCfxPool = NULL;

    cfxpool_Expand(128);
    
    g_pcAudio.m_TopCfxId        = 0;
}
...
void        cfx_RemoveFromUpdate(cfx *pCfx)
{
    ...
    {
        g_pcAudio.m_pRootCfx = pCfx->m_pNext;
    }
}
...
//		return NULL;
    ...
    pSample = pccontainer_Find(SampleId);
    ASSERT(pSample);
...
//    pCfx->m_OriginalState.m_RearVol = pSample->m_pAttributes->m_Volume;
	pCfx->m_OriginalState.m_Position = pSample->m_pAttributes->m_Position;
...
//    pCfx->m_State.m_RearVol = 0;
    ...
    else
    {
        ...
        //
        pParent = pccontainer_Find(pSample->m_pAttributes->m_OwnerId);
        ASSERT(pParent);
...
//        pCfx->m_OriginalState.m_RearVol = pParent->m_pAttributes->m_Volume;
		pCfx->m_OriginalState.m_Position = pParent->m_pAttributes->m_Position;
...
//        pCfx->m_pElements[i].m_Flags = pCfx->m_State.m_Flags;
//        pCfx->m_Elements[i].m_pAttributes->m_Position = pCfx->m_State.m_Position;
    }
...
#ifdef  sansari
//    x_DebugMsg("Cfx Alloc\n");
#endif
    if (pCfx->m_pElements->m_pAttributes->m_Type < 0)
        ASSERT(FALSE);
    return pCfx;
}
//-----------------------------------------------------------------------------
...
{
    pcaudio_DupState(&pCfx->m_State,&pCfx->m_OriginalState,pState);
...
//        ASSERT(index<pCfx->m_ElementCount);
        ...
        {
            pcaudio_DupState(&State,pParentState,&pCfx->m_State);
