...
// This little header is only used on gamecube.
typedef struct 
{
    // start context
    u16 coef[16];
...
typedef struct s_cfx_element_attributes
{
    ...
    s32         m_MediaFile;
    char        m_Name[60];             // The name of the file.
...
typedef struct s_cfx_element
{
    ...
    f32         m_SurroundDelay;
    f32         m_ExpireDelay;
    f32         m_Delay;                // Delay in milliseconds
    ...
    struct s_cfx_element *m_pParent;    // The parent of the element.
    s32         m_Length;
    s32         m_SpuLocation;          // Address within SPU
    adpcm_hdr   m_AdpcmHeader;          // Header required for proper ADPCM decode. Size is hardcoded here! Yuk!
