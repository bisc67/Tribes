...
void        voicestream_PeriodicUpdate(void)
{
    ...
    while (1)
    {
        ...
        pRequest = (stream_request *)g_gcnAudio.m_pqPendingStreamRequests->Recv(MQ_BLOCK);
        ASSERT(pRequest);
...
// Create our stream list
    g_gcnAudio.m_pqFreeStreams = new xmesgq(nBuffers);
...
// Message queue for completed pre-read blocks
        ...
        g_gcnAudio.m_pqFreeStreams->Send(pStreams,MQ_BLOCK);
        pStreams++;
    }
    g_gcnAudio.m_pStreamBuffers =(u8*)x_malloc(AUD_STREAM_BUFFER_SIZE * AUD_MAX_STREAMS * AUD_STREAM_READAHEAD);
    ASSERT(g_gcnAudio.m_pStreamBuffers);
// Message queue for the currently available buffers
    g_gcnAudio.m_pqAvailableStreamRequests = new xmesgq(AUD_MAX_STREAMS * AUD_STREAM_READAHEAD);
// Message queue for the currently outstanding read requests
    g_gcnAudio.m_pqPendingStreamRequests = new xmesgq(AUD_MAX_STREAMS * AUD_STREAM_READAHEAD);
    for (i=0;i<AUD_MAX_STREAMS * AUD_STREAM_READAHEAD;i++)
    {
        g_gcnAudio.m_StreamRequests[i].m_pData = &g_gcnAudio.m_pStreamBuffers[i*AUD_STREAM_BUFFER_SIZE];
        g_gcnAudio.m_pqAvailableStreamRequests->Send(&g_gcnAudio.m_StreamRequests[i],MQ_BLOCK);
    }
    g_gcnAudio.m_pStreamThread = new xthread(voicestream_PeriodicUpdate,"VoiceStream periodic Update",8192,2);
...
#endif
    pStream = (voice_stream*)g_gcnAudio.m_pqFreeStreams->Recv(MQ_NOBLOCK);
    if (!pStream) 
        return NULL;
...
xbool        voicestream_Free(voice_stream *pStream)
{
    ...
    while (pStream->m_ReadaheadInProgress)
    {
        ...
        if (pReq)
        {
            status = g_gcnAudio.m_pqAvailableStreamRequests->Send(pReq,MQ_NOBLOCK);
            ASSERT(status);
    ...
    g_gcnAudio.m_pqFreeStreams->Send(pStream,MQ_BLOCK);
    return TRUE;
}
...
xbool voicestream_UpdateReadahead(voice_stream *pStream)
{
         ...
         (pStream->m_ReadaheadRemaining < (pStream->m_BufferCount/2) ))
    {
        ...
        {
            pRequest = (stream_request *)g_gcnAudio.m_pqAvailableStreamRequests->Recv(MQ_NOBLOCK);
            ...
            g_gcnAudio.m_pqPendingStreamRequests->Send(pRequest,MQ_BLOCK);
            pStream->m_ReadaheadInProgress++;
...
//        printf("Block request delayed, offset=%d, remain=%d\n",pStream->m_PlayRemaining,remaining);
    ...
    if (Stereo)
    {
        ...
        pStream->m_PlayRemaining--;
        pStream->m_ReadaheadInProgress--;
        g_gcnAudio.m_pqAvailableStreamRequests->Send(pSiblingRequest,MQ_BLOCK);
    }
...
//            spu_InitVoice(pStream->m_pSibling->m_pVoice->m_Id,pStream->m_pSibling->m_SpuBuffer,pStream->m_pOwner->m_pAttributes->m_ADSR1,pStream->m_pOwner->m_pAttributes->m_ADSR2);
    ...
    status = g_gcnAudio.m_pqAvailableStreamRequests->Send(pRequest,MQ_NOBLOCK);
    ASSERT(status);
    return TRUE;
}
