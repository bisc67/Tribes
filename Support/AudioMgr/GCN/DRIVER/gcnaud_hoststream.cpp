...
static cfx *hs_Init(s32 index,s32 Pitch)
{
    ...
    pElement = &g_gcnAudio.m_StreamerElement[index];
    ...
    pElement->m_pAttributes = &g_gcnAudio.m_StreamerAttributes[index];
...
{
    s32 status;
    cfx_RemoveFromUpdate(g_gcnAudio.m_pStreamerCfx[index]);
    while (1)
    {
        status = voicestream_Free(g_gcnAudio.m_StreamerElement[index].m_pStream);
        if (status)
            break;
        x_DelayThread(10);
    }
    cfx_Free(g_gcnAudio.m_pStreamerCfx[index]);
}
...
void hoststream_Init(s32 Pitch)
{
    ...
    g_gcnAudio.m_pStreamerCfx[0] = hs_Init(0,Pitch);
    g_gcnAudio.m_pStreamerCfx[1] = hs_Init(1,Pitch);
...
s32 hoststream_Update(u8 *pData,s32 Length,s32 Index)
{
    ...
    pStreamRequest = (stream_request *)g_gcnAudio.m_pqAvailableStreamRequests->Recv(MQ_NOBLOCK);
    if (pStreamRequest)
    {
        pCfx = g_gcnAudio.m_pStreamerCfx[Index];
        ...
        if (!SimpleReturn)
        {
            g_gcnAudio.m_pqAvailableStreamRequests->Send(pStreamRequest,MQ_BLOCK);
        }
        else
        {
