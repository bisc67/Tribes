...
char* FormatIntoStringBuffer( const char* pFormatString, x_va_list Args )
{
    ...
    *pNChars = x_vsprintf( pResult, pFormatString, Args );
    ...
    ASSERT( pGlobals->NextOffset < X_GLOBAL_STRING_BUFFER_SIZE );
...
void xstring::EnsureCapacity( s32 Capacity )
{
    s32  NewBufferSize;
    ...
    NewBufferSize = ALIGN_16( Capacity + 9 );
    Capacity      = NewBufferSize - 9;

    if( (NewBufferSize > BUFFER_SIZE) || (Capacity > STRING_LENGTH+1) )
    {
...
xbool operator < ( const char* S1, const xstring& S2 )
{
    ...
    if( Relate == 0 )
    {
        ...
        // S1 < S2 only if it is shorter.
        return( x_strlen(S1) < S2.GetLength() );
    }
    else
    {
        return( Relate < 0 );
...
xbool xstring::SaveFile( const char* pFileName ) const
{
/*
    s32     Written;
    ...
    Written = x_fwrite( m_pData, 1, STRING_LENGTH, pFile );
    x_fclose( pFile );

    return( Written == STRING_LENGTH );
*/
    s32     Written = 0;
    xstring TempStr( *this );
...
void xwstring::EnsureCapacity( s32 Capacity )
{
    s32  NewBufferSize;
    ...
    NewBufferSize = ALIGN_16( (Capacity << 1) + 10 );
    Capacity      = (NewBufferSize - 10) >> 1;
