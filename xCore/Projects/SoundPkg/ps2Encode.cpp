...
//
s32 ps2EncodeStream(s16 *pSource,s16 *pDest,s32 BlockSkip,s32 Length,s32 LoopStart,s32 LoopEnd)
{
    s32 EncodeLength;
...
s32 ps2_EncodeToAdpcm(s32 Length,s16 *pOutBuffer,t_DecodeHeader *pHeader)
{
    ...
    ASSERT(GetTargetPlatform()==TARGET_TYPE_PS2);
    ...
    if (pHeader->Flags & AUDFLAG_STEREO)
    {
        length = ps2EncodeStream(pHeader->pLeft,pOutBuffer,2048,pHeader->Length / 2,pHeader->LoopStart,pHeader->LoopEnd);
        length += ps2EncodeStream(pHeader->pRight,pOutBuffer+1024,2048,pHeader->Length / 2,pHeader->LoopStart,pHeader->LoopEnd);
        x_printf("Stereo sample, length=%08x, encoded length=%08x\n",pHeader->Length,length);
    }
    else
    {
        length = ps2EncodeStream(pHeader->pLeft,pOutBuffer,0,pHeader->Length,pHeader->LoopStart,pHeader->LoopEnd);
    }
