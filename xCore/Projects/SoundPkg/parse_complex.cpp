...
sample_body *FindSample(token_stream &Tok,char *pFilename,parse_output &Output,xbool ForceLooped,s32 type)
{
    ...
    // We want to get the looping flags out of the aif file.
    if (GetTargetPlatform() == TARGET_TYPE_PC)
    {
        s32 slength = x_strlen(pFilename);

        // This is the effect package, by reading in the aif files for PS2 we get the looping data out.
        if (Output.m_PackageId == 2)
        {
            // Hacking it right.
            // We want to know the looping info from the aif file.
            pFilename[slength-1] = 'f';
            pFilename[slength-2] = 'i';
            pFilename[slength-3] = 'a';

        }
      
        // This is the music package, its going to be mp3 so change the extension to wav, that will make it read the wav 
        // file for PS2.
        if (Output.m_PackageId == 3)
        {
            // Hacking it right.
            // We want to know the looping info from the aif file.
            pFilename[slength-1] = 'v';
            pFilename[slength-2] = 'a';
            pFilename[slength-3] = 'w';

        }

        // This is the voice package, don't have any loop here but this will make sure that the DecodeToPcm won't crash.
        if (Output.m_PackageId == 4)
        {
            // Hacking it right.
            // We want to know the looping info from the aif file.
            pFilename[slength-1] = 'f';
    ...
    if (GetTargetPlatform() == TARGET_TYPE_PC)
    {
        // We store the name of the file and also the loopstart and loopend, wether the sample is looped or not.
        pSample->m_Length       = x_strlen(pSample->m_Filename) + (2*sizeof(s32));
    }
    else
    {    
    ...
    case TARGET_TYPE_GCN:
        ...
        break;
	case TARGET_TYPE_PC:
		CompressedLength = pc_Encode( pSample->m_Length, pSample->m_pData, (s8 *)pSample->m_Filename, &Header, Output.m_PackageId);
		break;
    default:
