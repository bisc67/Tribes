...
IDirect3DVertexBuffer8*     m_pVertexBuffer3d[NUM_VERTEX_BUFFERS];  // Array of vertex buffer pointers
...
IDirect3DVertexBuffer8*     m_pVertexBuffer2d[NUM_VERTEX_BUFFERS];  // Array of vertex buffer pointers
...
IDirect3DIndexBuffer8*      m_pIndexQuads;                          // Index array for rendering Quads
...
void draw_Init( void )
{
    ...
    m_iActiveBuffer = 0;
    Error = m_pVertexBuffer3d[m_iActiveBuffer]->Lock( 0, 0, (byte**)&m_pActiveBuffer3dStart, D3DLOCK_NOSYSLOCK | D3DLOCK_DISCARD );
    ASSERT( Error == 0 );
    Error = m_pVertexBuffer2d[m_iActiveBuffer]->Lock( 0, 0, (byte**)&m_pActiveBuffer2dStart, D3DLOCK_NOSYSLOCK | D3DLOCK_DISCARD );
    ASSERT( Error == 0 );
    ...
    {
        // SB - D3DPOOL_MANAGED so we can recover from losing the d3d device
        ...
        Error = m_pIndexQuads->Lock( 0, 0, (byte**)&pIndex, 0 );
        ASSERT( Error == 0 );
...
void draw_Dispatch( void )
{
    ...
    if( m_iVertex != 0 )
    {
        ...
        for( i=0 ; i<eng_GetNActiveViews() ; i++ )
        {
            ...
            // Set stream source
            if( m_Is2D )
                g_pd3dDevice->SetStreamSource( 0, m_pVertexBuffer2d[m_iActiveBuffer], sizeof(drawvertex2d) );
            else
                g_pd3dDevice->SetStreamSource( 0, m_pVertexBuffer3d[m_iActiveBuffer], sizeof(drawvertex3d) );

            // Device okay? 
            // Without this, D3D crashes because the vertex buffers could be trashes (when alt+tab pressed!)
            if (g_pd3dDevice->TestCooperativeLevel() == D3D_OK)
            {
                ...
                case DRAW_RECTS:
                case DRAW_SPRITES:
                    g_pd3dDevice->SetIndices( m_pIndexQuads, 0 );
                    g_pd3dDevice->DrawIndexedPrimitive( D3DPT_TRIANGLELIST, 0, m_iVertex, 0, m_iVertex/2 );
                    break;
                }
            }
        }
...
//    g_pd3dDevice->SetRenderState( D3DRS_CULLMODE, D3DCULL_NONE );
    ...
    g_pd3dDevice->SetRenderState( D3DRS_ZBIAS, m_ZBias );
