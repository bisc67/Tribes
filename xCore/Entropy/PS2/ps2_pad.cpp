...
// define this to allow keyboard and mouse inputs
//#define ENABLE_KEYBOARD

#ifdef ENABLE_KEYBOARD
#include "iop/usb.hpp"
#include "iop/usbdefs.hpp"
#endif
struct pad
{
    u32             buttons;
...
static xbool    s_UpdatePrev;

static pads s_ReadState;
static pads s_PrevReadState;
s32 s_vSyncSemaphore = -1;
...
void input_Init(void)
{
    ...
    x_memset(&s_PrevReadState,0,sizeof(s_PrevReadState));
    s_UpdatePrev = TRUE;
...
#ifdef ENABLE_KEYBOARD
    usb_ReadDevices(s_pCurrentPad->Usb);
    if (s_pPreviousPad)
        s_pCurrentPad->OldUsb = s_pPreviousPad->Usb;
#endif
...
{   
    if( (GadgetID >= INPUT_KBD__BEGIN) && (GadgetID < INPUT_KBD__END))
    {
        ...
        ASSERT( (ControllerID >=0) && (ControllerID < MAX_USB_KEYBOARD));
        index = GadgetID - INPUT_KBD__BEGIN;
        bit = 1<<(index & 31);
        index = index / 32;
        return ( (s_ReadState.Usb.Keyboard[ControllerID].pressed[index] & bit)!=0);
#else
        return 0;
#endif
    }
    ...
    if( (GadgetID >= INPUT_MOUSE__DIGITAL) && (GadgetID < INPUT_MOUSE__ANALOG) )
    {
        ...
        index = GadgetID - INPUT_MOUSE__DIGITAL;
        bit   = (1<<index);
        return ( (s_ReadState.Usb.Mouse[ControllerID].Buttons & bit)!=0);
#else
        return 0;
#endif
    }
...
#ifdef ENABLE_KEYBOARD
            return s_ReadState.Usb.Mouse[ControllerID].is_present;
#else
            return FALSE;
#endif
            break;
...
#ifdef ENABLE_KEYBOARD
            return s_ReadState.Usb.Keyboard[ControllerID].is_present;
#else
            return FALSE;
#endif
            break;
...
xbool input_WasPressed( input_gadget GadgetID, s32 ControllerID )
{
    if ( (GadgetID >= INPUT_KBD__BEGIN) && (GadgetID < INPUT_KBD__END))
    {
        ...
        bit = 1<<(index & 31);
        index = index / 32;
        return ((( s_ReadState.Usb.Keyboard[ControllerID].pressed[index] & 
                  ~s_ReadState.OldUsb.Keyboard[ControllerID].pressed[index]) & 
                   bit) !=0);
#else
        return FALSE;
#endif
    ...
    {
#ifdef ENABLE_KEYBOARD
        ASSERT( (ControllerID >=0) && (ControllerID < MAX_USB_MOUSE));
        s32 index;
        s32 bit;

        index = GadgetID - INPUT_MOUSE__DIGITAL;
        bit   = 1 << index ;
        return ((( s_ReadState.Usb.Mouse[ControllerID].Buttons & 
                  ~s_ReadState.OldUsb.Mouse[ControllerID].Buttons) &
                  bit) !=0);
#else
        return FALSE;
#endif
    ...
    s32 Bit = GadgetID - INPUT_PS2_BTN_L2;
    s32 Buttons = s_ReadState.Pad[ControllerID].buttons;
    s32 PrevButtons = s_PrevReadState.Pad[ControllerID].buttons;
...
f32 input_GetValue( input_gadget GadgetID, s32 ControllerID )
{
    ...
    {
#ifdef ENABLE_KEYBOARD
        case INPUT_MOUSE_X_REL:         return s_ReadState.Usb.Mouse[ControllerID].X - s_ReadState.OldUsb.Mouse[ControllerID].X;    
        case INPUT_MOUSE_Y_REL:         return s_ReadState.Usb.Mouse[ControllerID].Y - s_ReadState.OldUsb.Mouse[ControllerID].Y;    
        case INPUT_MOUSE_WHEEL_REL:     return s_ReadState.Usb.Mouse[ControllerID].Wheel - s_ReadState.OldUsb.Mouse[ControllerID].Wheel ;

        case INPUT_MOUSE_X_ABS:         return s_ReadState.Usb.Mouse[ControllerID].X;    
        case INPUT_MOUSE_Y_ABS:         return s_ReadState.Usb.Mouse[ControllerID].Y;       
        case INPUT_MOUSE_WHEEL_ABS:     return s_ReadState.Usb.Mouse[ControllerID].Wheel;
#else
...
{
    if (s_vSyncSemaphore != -1)
	{
		s_vSyncCounter++;
		iSignalSema(s_vSyncSemaphore);
	}
}
...
//            x_DebugMsg("Intensity : %2.2f, actuator 0: %d, actuator 1: %d\n",pFeedback->Intensity,pFeedback->Actuator[0],pFeedback->Actuator[1]);
