 ...
 */
xpng_charp xpng_decompress_chunk(xpng_structp xpng_ptr, int comp_type,
                              xpng_charp chunkdata, xpng_size_t chunklength,
                              xpng_size_t prefix_size)
{
   static char msg[] = "Error decoding compressed text";
   xpng_charp text = NULL;
   xpng_size_t text_size = (chunklength - prefix_size);
      ...
      while (xpng_ptr->zstream.avail_in)
      {
         ...
         if (!xpng_ptr->zstream.avail_out || ret == Z_STREAM_END)
         {
            if (text == NULL)
            {
               ...
               text = (xpng_charp)xpng_malloc(xpng_ptr, text_size + 1);
            ...
            else
            {
               ...
               tmp = text;
               text = (xpng_charp)xpng_malloc(xpng_ptr, (xpng_uint_32)(text_size +
   ...
   else /* if (comp_type >= PNG_TEXT_COMPRESSION_LAST) */
   {
      ...
      /* Copy what we can of the error message into the text chunk */
      text_size = (xpng_size_t)(chunklength - (text - chunkdata));
      text_size = sizeof(msg) > text_size ? text_size : sizeof(msg);
      xpng_memcpy(text, msg, text_size);
   }
...
xpng_handle_IHDR(xpng_structp xpng_ptr, xpng_infop info_ptr, xpng_uint_32 length)
{
   ...
   if (xpng_ptr->mode != PNG_BEFORE_IHDR)
...
xpng_handle_PLTE(xpng_structp xpng_ptr, xpng_infop info_ptr, xpng_uint_32 length)
{
   ...
   num = (int)length / 3;
   palette = (xpng_colorp)xpng_zalloc(xpng_ptr, (uInt)num, sizeof (xpng_color));
   xpng_ptr->flags |= PNG_FLAG_FREE_PALETTE;
   for (i = 0; i < num; i++)
   {
      xpng_byte buf[3];
   ...
   xpng_ptr->num_palette = (xpng_uint_16)num;
   ...
   if (xpng_ptr->color_type == PNG_COLOR_TYPE_PALETTE)
   {
      ...
      {
         if (xpng_ptr->num_trans > xpng_ptr->num_palette)
         {
            xpng_warning(xpng_ptr, "Truncating incorrect tRNS chunk length");
            xpng_ptr->num_trans = xpng_ptr->num_palette;
         }
      }
   }
#endif
...
xpng_handle_cHRM(xpng_structp xpng_ptr, xpng_infop info_ptr, xpng_uint_32 length)
{
   ...
   xpng_crc_read(xpng_ptr, buf, 4);
   int_x_white = (xpng_fixed_point)xpng_get_uint_32(buf);

   xpng_crc_read(xpng_ptr, buf, 4);
   int_y_white = (xpng_fixed_point)xpng_get_uint_32(buf);
   ...
   xpng_crc_read(xpng_ptr, buf, 4);
   int_x_red = (xpng_fixed_point)xpng_get_uint_32(buf);

   xpng_crc_read(xpng_ptr, buf, 4);
   int_y_red = (xpng_fixed_point)xpng_get_uint_32(buf);
   ...
   xpng_crc_read(xpng_ptr, buf, 4);
   int_x_green = (xpng_fixed_point)xpng_get_uint_32(buf);

   xpng_crc_read(xpng_ptr, buf, 4);
   int_y_green = (xpng_fixed_point)xpng_get_uint_32(buf);
   ...
   xpng_crc_read(xpng_ptr, buf, 4);
   int_x_blue = (xpng_fixed_point)xpng_get_uint_32(buf);

   xpng_crc_read(xpng_ptr, buf, 4);
   int_y_blue = (xpng_fixed_point)xpng_get_uint_32(buf);
...
/* Note: this does not properly handle chunks that are > 64K under DOS */
{
   ...
   xpng_charp profile;
   xpng_uint_32 skip = 0;
   xpng_size_t slength, prefix_length;
   ...
   {
      xpng_free(xpng_ptr, chunkdata);
      xpng_error(xpng_ptr, "malformed iCCP chunk");
   }
   ...
   xpng_set_iCCP(xpng_ptr, info_ptr, chunkdata, compression_type,
                chunkdata + prefix_length, xpng_strlen(chunkdata + prefix_length));
   xpng_free(xpng_ptr, chunkdata);
}
...
/* Note: this does not properly handle chunks that are > 64K under DOS */
{
   ...
   slength = (xpng_size_t)length;
   xpng_crc_read(xpng_ptr, chunkdata, slength);
   ...
   {
      xpng_free(xpng_ptr, chunkdata);
      xpng_error(xpng_ptr, "malformed sPLT chunk");
   }
   ...
   {
      xpng_free(xpng_ptr, chunkdata);
      xpng_error(xpng_ptr, "sPLT chunk has bad length");
   }

   new_palette.nentries = data_length / entry_size;
   new_palette.entries = (xpng_spalette_entryp)xpng_malloc(
       xpng_ptr, new_palette.nentries * sizeof(xpng_spalette_entry));

   for (i = 0; i < new_palette.nentries; i++)
   {
      xpng_spalette_entryp pp = new_palette.entries + i;
   ...
   xpng_set_spalettes(xpng_ptr, info_ptr, &new_palette, 1);
...
xpng_handle_tRNS(xpng_structp xpng_ptr, xpng_infop info_ptr, xpng_uint_32 length)
{
   ...
   if (xpng_ptr->color_type == PNG_COLOR_TYPE_PALETTE)
   {
      ...
      else if (length > xpng_ptr->num_palette)
      {
      ...
      xpng_ptr->trans = (xpng_bytep)xpng_malloc(xpng_ptr, length);
      xpng_ptr->flags |= PNG_FLAG_FREE_TRANS;
      xpng_crc_read(xpng_ptr, xpng_ptr->trans, (xpng_size_t)length);
   ...
   xpng_set_tRNS(xpng_ptr, info_ptr, xpng_ptr->trans, xpng_ptr->num_trans,
...
xpng_handle_hIST(xpng_structp xpng_ptr, xpng_infop info_ptr, xpng_uint_32 length)
{
   ...
   if (length != (xpng_uint_32)(2 * xpng_ptr->num_palette))
   {
   ...
   num = (int)length / 2 ;
   xpng_ptr->hist = (xpng_uint_16p)xpng_malloc(xpng_ptr,
      (xpng_uint_32)(num * sizeof (xpng_uint_16)));
   ...
   for (i = 0; i < num; i++)
   {
      ...
      xpng_crc_read(xpng_ptr, buf, 2);
      xpng_ptr->hist[i] = xpng_get_uint_16(buf);
   }

   if (xpng_crc_finish(xpng_ptr, 0))
      return;

   xpng_set_hIST(xpng_ptr, info_ptr, xpng_ptr->hist);
}
#endif
...
xpng_handle_pCAL(xpng_structp xpng_ptr, xpng_infop info_ptr, xpng_uint_32 length)
{
   ...
   xpng_debug1(2, "Allocating and reading pCAL chunk data (%d bytes)\n",
      length + 1);
   purpose = (xpng_charp)xpng_malloc(xpng_ptr, length + 1);
   slength = (xpng_size_t)length;
   ...
   params = (xpng_charpp)xpng_malloc(xpng_ptr, (xpng_uint_32)(nparams
      *sizeof(xpng_charp))) ;
...
xpng_handle_sCAL(xpng_structp xpng_ptr, xpng_infop info_ptr, xpng_uint_32 length)
{
   ...
   xpng_debug1(2, "Allocating and reading sCAL chunk data (%d bytes)\n",
      length + 1);
   buffer = (xpng_charp)xpng_malloc(xpng_ptr, length + 1);
   slength = (xpng_size_t)length;
   ...
   width = strtod(ep, &vp);
   if (*vp)
       xpng_error(xpng_ptr, "malformed width string in sCAL chunk");
#else
#ifdef PNG_FIXED_POINT_SUPPORTED
   swidth = (xpng_charp)xpng_malloc(xpng_ptr, strlen(ep) + 1);
   xpng_memcpy(swidth, ep, (xpng_size_t)strlen(ep));
#endif
#endif
   ...
   height = strtod(ep, &vp);
   if (*vp)
       xpng_error(xpng_ptr, "malformed height string in sCAL chunk");
#else
#ifdef PNG_FIXED_POINT_SUPPORTED
   sheight = (xpng_charp)xpng_malloc(xpng_ptr, strlen(ep) + 1);
   xpng_memcpy(sheight, ep, (xpng_size_t)strlen(ep));
#endif
#endif
...
xpng_handle_tEXt(xpng_structp xpng_ptr, xpng_infop info_ptr, xpng_uint_32 length)
{
   ...
   key = (xpng_charp)xpng_malloc(xpng_ptr, length + 1);
   slength = (xpng_size_t)length;
   ...
   xpng_set_text(xpng_ptr, info_ptr, text_ptr, 1);
...
xpng_handle_zTXt(xpng_structp xpng_ptr, xpng_infop info_ptr, xpng_uint_32 length)
{
   ...
   xpng_size_t slength, prefix_len;
...
xpng_handle_iTXt(xpng_structp xpng_ptr, xpng_infop info_ptr, xpng_uint_32 length)
{
   ...
   text_ptr->itxt_length = xpng_strlen(text_ptr->text);
...
xpng_handle_unknown(xpng_structp xpng_ptr, xpng_infop info_ptr, xpng_uint_32 length)
{
   ...
   if (!(xpng_ptr->chunk_name[0] & 0x20))
   {
      if(xpng_handle_as_unknown(xpng_ptr, xpng_ptr->chunk_name) !=
           ...
           && xpng_ptr->read_user_chunk_fn == (xpng_user_chunk_ptr)NULL
#endif
        )
   ...
   if (xpng_ptr->flags & PNG_FLAG_KEEP_UNKNOWN_CHUNKS)
   {
       ...
       strcpy((xpng_charp)chunk.name, (xpng_charp)xpng_ptr->chunk_name);
       chunk.data = (xpng_bytep)xpng_malloc(xpng_ptr, length);
       xpng_crc_read(xpng_ptr, chunk.data, length);
       chunk.size = length;
#if defined(PNG_READ_USER_CHUNKS_SUPPORTED)
       if(xpng_ptr->read_user_chunk_fn != (xpng_user_chunk_ptr)NULL)
       {
...
xpng_combine_row
   ...
   (xpng_structp xpng_ptr, xpng_bytep row, int mask)
{
...
xpng_do_read_interlace_c
#else
xpng_do_read_interlace
#endif /* PNG_HAVE_ASSEMBLER_READ_INTERLACE */
   (xpng_row_infop row_info, xpng_bytep row, int pass,
...
xpng_read_filter_row_c
#else
xpng_read_filter_row
#endif /* PNG_HAVE_ASSEMBLER_READ_FILTER_ROW */
   (xpng_structp xpng_ptr, xpng_row_infop row_info, xpng_bytep row,
   xpng_bytep prev_row, int filter)
{
   xpng_debug(1, "in xpng_read_filter_row\n");
   xpng_debug2(2,"row = %d, filter = %d\n", xpng_ptr->row_number, filter);
   switch (filter)
   {
...
xpng_read_start_row(xpng_structp xpng_ptr)
{
...
#endif
   xpng_ptr->row_buf = (xpng_bytep)xpng_malloc(xpng_ptr, row_bytes);
   ...
   xpng_debug1(3, "width = %d,\n", xpng_ptr->width);
   xpng_debug1(3, "height = %d,\n", xpng_ptr->height);
   xpng_debug1(3, "iwidth = %d,\n", xpng_ptr->iwidth);
   xpng_debug1(3, "num_rows = %d\n", xpng_ptr->num_rows);
   xpng_debug1(3, "rowbytes = %d,\n", xpng_ptr->rowbytes);
   xpng_debug1(3, "irowbytes = %d,\n", xpng_ptr->irowbytes);
