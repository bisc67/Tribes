   ...
   xpng_malloc_ptr malloc_fn, xpng_free_ptr free_fn)
{
   ...
   xpng_set_read_fn(xpng_ptr, NULL, NULL);
...
xpng_read_init(xpng_structp xpng_ptr)
{
   ...
   xpng_debug(1, "in xpng_read_init\n");
...
xpng_read_info(xpng_structp xpng_ptr, xpng_infop info_ptr)
{
   ...
   for(;;)
   {
      ...
      xpng_debug2(0, "Reading %s chunk, length=%d.\n", xpng_ptr->chunk_name,
         length);
...
#endif
   int ret;
   xpng_debug2(1, "in xpng_read_row (row %d, pass %d)\n",
 ...
 *
 * [*] xpng_handle_alpha() does not exist yet, as of libpng version 1.0.5m.
 */
   ...
   xpng_bytepp display_row, xpng_uint_32 num_rows)
{
   ...
   if (rp != NULL && dp != NULL)
      ...
      {
         xpng_bytep rptr = *rp;
         xpng_read_row(xpng_ptr, rptr, NULL);
         rp++;
      }
   else if(dp != NULL)
      for (i = 0; i < num_rows; i++)
      {
         xpng_bytep dptr = *dp;
         xpng_read_row(xpng_ptr, NULL, dptr);
         dp++;
      }
}
...
xpng_read_image(xpng_structp xpng_ptr, xpng_bytepp image)
{
   ...
   for (j = 0; j < pass; j++)
   {
      ...
      {
         xpng_read_row(xpng_ptr, *rp, NULL);
         rp++;
      }
   }
}
   ...
   xpng_infopp end_info_ptr_ptr)
{
   ...
   {
#if defined(PNG_READ_TEXT_SUPPORTED)
      xpng_free(xpng_ptr, end_info_ptr->text);
#endif
#ifdef PNG_USER_MEM_SUPPORTED
      xpng_destroy_struct_2((xpng_voidp)end_info_ptr, free_fn);
#else
      xpng_destroy_struct((xpng_voidp)end_info_ptr);
#endif
      *end_info_ptr_ptr = (xpng_infop)NULL;
   }
...
#endif
   if (xpng_ptr->flags & PNG_FLAG_FREE_PALETTE)
      xpng_zfree(xpng_ptr, xpng_ptr->palette);
#if defined(PNG_tRNS_SUPPORTED) || \
    defined(PNG_READ_EXPAND_SUPPORTED) || defined(PNG_READ_BACKGROUND_SUPPORTED)
   if (xpng_ptr->flags & PNG_FLAG_FREE_TRANS)
...
void xpng_read_png(xpng_structp xpng_ptr, xpng_infop info_ptr,
			   int transforms,
			   voidp params)
{
   int row;
   ...
   if (transforms & PNG_TRANSFORM_EXPAND)
       if (xpng_get_valid(xpng_ptr, info_ptr, PNG_INFO_tRNS))
   ...
   for (row = 0; row < (int)info_ptr->height; row++)
       info_ptr->row_pointers[row] = xpng_malloc(xpng_ptr, 
