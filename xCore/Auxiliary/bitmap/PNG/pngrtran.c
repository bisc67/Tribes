   ...
   int full_dither)
{
   ...
   if (num_palette > maximum_colors)
   {
      if (histogram != NULL)
      {
         ...
         sort = (xpng_bytep)xpng_malloc(xpng_ptr, (xpng_uint_32)(num_palette
            * sizeof (xpng_byte)));
      ...
      else
      {
         ...
         int num_new_palette;
         xpng_dsortpp hash;
         xpng_bytep index_to_palette;
            /* where the original index currently is in the palette */
         xpng_bytep palette_to_index;
            /* which original index points to this palette color */

         /* initialize palette index arrays */
         index_to_palette = (xpng_bytep)xpng_malloc(xpng_ptr,
            (xpng_uint_32)(num_palette * sizeof (xpng_byte)));
         palette_to_index = (xpng_bytep)xpng_malloc(xpng_ptr,
            (xpng_uint_32)(num_palette * sizeof (xpng_byte)));

         /* initialize the sort array */
         for (i = 0; i < num_palette; i++)
         {
            index_to_palette[i] = (xpng_byte)i;
            palette_to_index[i] = (xpng_byte)i;
         }
...
/*         xpng_memset(hash, 0, 769 * sizeof (xpng_dsortp)); */
            ...
            for (i = 0; i < num_new_palette - 1; i++)
            {
               ...
               for (j = i + 1; j < num_new_palette; j++)
               {
                  ...
                  if (d <= max_d)
                  {
                     ...
                     t = (xpng_dsortp)xpng_malloc(xpng_ptr, (xpng_uint_32)(sizeof
                         (xpng_dsort)));
                     t->next = hash[d];
                     t->left = (xpng_byte)i;
                     t->right = (xpng_byte)j;
               ...
               if (hash[i] != NULL)
               {
                  ...
                  for (p = hash[i]; p; p = p->next)
                  {
                        ...
                        (int)index_to_palette[p->right] < num_new_palette)
                     {
                        ...
                        index_to_palette[j] = (xpng_byte)num_new_palette;
                        palette_to_index[num_new_palette] = (xpng_byte)j;
                     }
         ...
         xpng_free(xpng_ptr, palette_to_index);
         xpng_free(xpng_ptr, index_to_palette);
      }
...
#endif
   {
      xpng_byte red_byte = (xpng_byte)((float)red*255.0 + 0.5);
      xpng_byte green_byte = (xpng_byte)((float)green*255.0 + 0.5);
      ...
      xpng_ptr->rgb_to_gray_red_coeff   = red_byte;
      xpng_ptr->rgb_to_gray_green_coeff = green_byte;
      xpng_ptr->rgb_to_gray_blue_coeff  = (xpng_byte)(255-red_byte-green_byte);
   }
}
#endif
...
xpng_do_read_transformations(xpng_structp xpng_ptr)
{
   ...
   if (xpng_ptr->row_buf == NULL)
   {
      ...
      sprintf(msg, "NULL row buffer for row %d, pass %d", xpng_ptr->row_number,
         xpng_ptr->pass);
...

{
   ...
   if (
      ...
      (row_info->color_type & PNG_COLOR_MASK_COLOR))
   {
      xpng_byte rc = xpng_ptr->rgb_to_gray_red_coeff;
      xpng_byte gc = xpng_ptr->rgb_to_gray_green_coeff;
      xpng_byte bc = xpng_ptr->rgb_to_gray_blue_coeff;
         ...
         if (row_info->bit_depth == 8)
         {
...
#endif
            {
               ...
               for (i = 0; i < row_width; i++)
               {
                  ...
                  {
                     rgb_error |= 1;
                     *(dp++) = (xpng_byte)((rc*red+gc*green+bc*blue)>>8);
                  }
                  else
                     *(dp++) = *(sp-1);
               }
         ...
         else /* RGB bit_depth == 16 */
         {
...
#endif
            {
               ...
               for (i = 0; i < row_width; i++)
               {
                  ...
                  if(red != green || red != blue)
                     rgb_error |= 1;
                  gray16  = (xpng_uint_16)((rc*red + gc*green + bc*blue)>>8);
                   ...
                   gamma_16_to_1 != NULL)
               {
                  ...
                  for (i = 0; i < row_width; i++, sp += 8, dp += 6)
                  {
                     ...
                     else
                     {
                        ...
                        xpng_composite_16(w, v, a, background->red);
                        x = gamma_16_from_1[((w&0xff) >> gamma_shift)][w >> 8];
                        *dp = (xpng_byte)((x >> 8) & 0xff);
                        *(dp + 1) = (xpng_byte)(x & 0xff);
                        v = gamma_16_to_1[*(sp + 3) >> gamma_shift][*(sp + 2)];
                        xpng_composite_16(w, v, a, background->green);
                        x = gamma_16_from_1[((w&0xff) >> gamma_shift)][w >> 8];
                        *(dp + 2) = (xpng_byte)((x >> 8) & 0xff);
                        *(dp + 3) = (xpng_byte)(x & 0xff);
                        v = gamma_16_to_1[*(sp + 5) >> gamma_shift][*(sp + 4)];
                        xpng_composite_16(w, v, a, background->blue);
                        x = gamma_16_from_1[(w & 0xff) >> gamma_shift][w >> 8];
