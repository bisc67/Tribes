      ...
      switch (xpng_ptr->pass)
      {
         ...
         case 1:
         {
            ...
            if (xpng_ptr->pass == 2)
            {
               ...
               {
                  xpng_push_have_row(xpng_ptr, NULL);
                  xpng_read_push_finish_row(xpng_ptr);
               }
            }
            break;
...
xpng_read_push_finish_row(xpng_structp xpng_ptr)
{
   ...
   const int xpng_pass_start[] = {0, 4, 0, 2, 0, 1, 0};

   /* offset to next interlace block */
   const int xpng_pass_inc[] = {8, 8, 4, 4, 2, 2, 1};

   /* start of interlace block in the y direction */
   const int xpng_pass_ystart[] = {0, 0, 4, 0, 2, 0, 1};

   /* offset to next interlace block in the y direction */
   const int xpng_pass_yinc[] = {8, 8, 8, 4, 4, 2, 2};

   /* Width of interlace block.  This is not currently used - if you need
    * it, uncomment it here and in png.h
   const int xpng_pass_width[] = {8, 4, 4, 2, 2, 1, 1};
   */

   /* Height of interlace block.  This is not currently used - if you need
    * it, uncomment it here and in png.h
   const int xpng_pass_height[] = {8, 8, 4, 4, 2, 2, 1};
   */
#endif
...
#if defined(PNG_READ_tEXt_SUPPORTED)
void
xpng_push_handle_tEXt(xpng_structp xpng_ptr, xpng_infop info_ptr, xpng_uint_32 length)
{
   if (xpng_ptr->mode == PNG_BEFORE_IHDR || xpng_ptr->mode & PNG_HAVE_IEND)
      {
   ...
   if (length > (xpng_uint_32)65535L) /* Can't hold the entire string in memory */
   {
...
#if defined(PNG_READ_zTXt_SUPPORTED)
void
xpng_push_handle_zTXt(xpng_structp xpng_ptr, xpng_infop info_ptr, xpng_uint_32 length)
{
   if (xpng_ptr->mode == PNG_BEFORE_IHDR || xpng_ptr->mode & PNG_HAVE_IEND)
      {
...
#if defined(PNG_READ_iTXt_SUPPORTED)
void
xpng_push_handle_iTXt(xpng_structp xpng_ptr, xpng_infop info_ptr, xpng_uint_32 length)
{
   if (xpng_ptr->mode == PNG_BEFORE_IHDR || xpng_ptr->mode & PNG_HAVE_IEND)
      {
 ...
 */
void
xpng_push_handle_unknown(xpng_structp xpng_ptr, xpng_infop info_ptr, xpng_uint_32 length)
{
