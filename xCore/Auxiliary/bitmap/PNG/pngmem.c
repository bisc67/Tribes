...
xpng_create_struct(int type)
{
   ...
   return (xpng_create_struct_2(type, NULL));
}

/* Alternate version of xpng_create_struct, for use with user-defined malloc. */
xpng_voidp
xpng_create_struct_2(int type, xpng_malloc_ptr malloc_fn)
{
...
{
#ifdef PNG_USER_MEM_SUPPORTED
   xpng_destroy_struct_2(struct_ptr, (xpng_free_ptr)NULL);
}
...
xpng_malloc(xpng_structp xpng_ptr, xpng_uint_32 size)
{
   ...
   if(xpng_ptr->malloc_fn != NULL)
       return ((xpng_voidp)(*(xpng_ptr->malloc_fn))(xpng_ptr, size));
   else
   ...
   if (size == (xpng_uint_32)65536L)
   {
      if (xpng_ptr->offset_table == NULL)
      {
         ...
         if (ret == NULL || ((xpng_size_t)ret & 0xffff))
         {
            ...
            {
               xpng_error(xpng_ptr, "Farmalloc didn't return normalized pointer");
            }
