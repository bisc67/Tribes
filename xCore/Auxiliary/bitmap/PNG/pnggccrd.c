...
//
// 19991024:
//  - fixed mmxsupport()/xpng_do_interlace() first-row bug
//     This one was severely weird:  even though mmxsupport() doesn't touch
//     ebx (where "row" pointer was stored), it nevertheless managed to zero
//     the register (even in static/non-fPIC code--see below), which in turn
//     caused xpng_do_interlace() to return prematurely on the first row of
...
void
xpng_read_filter_row_c(xpng_structp xpng_ptr, xpng_row_infop row_info,
   xpng_bytep row, xpng_bytep prev_row, int filter);
...
/*
fprintf(stderr, "GRR DEBUG:  xpng_combine_row() pixel_depth = %d, mask = 0x%02x, unmask = 0x%02x\n", xpng_ptr->row_info.pixel_depth, mask, ~mask);
fflush(stderr);
 */
   if (mask == 0xff)
   {
...
// preload        "movl      mask, %%edx     \n\t"
            ...
            else /* mmx _not supported - Use modified C routine */
            {
               ...
               register xpng_uint_32 final_val = xpng_ptr->width;
...
// preload        "movl      mask, %%edx      \n\t"
            ...
            {
               register xpng_uint_32 i;
               xpng_uint_32 initial_val = 2 * xpng_pass_start[xpng_ptr->pass];
                 // png.c:  xpng_pass_start[] = {0, 4, 0, 2, 0, 1, 0};
               register int stride = 2 * xpng_pass_inc[xpng_ptr->pass];
                 // png.c:  xpng_pass_inc[] = {8, 8, 4, 4, 2, 2, 1};
               register int rep_bytes = 2 * xpng_pass_width[xpng_ptr->pass];
                 // png.c:  xpng_pass_width[] = {8, 4, 4, 2, 2, 1, 1};
               register xpng_uint_32 final_val = 2 * xpng_ptr->width;
...
// preload        "movl      mask, %%edx      \n\t"
            ...
            {
               register xpng_uint_32 i;
               xpng_uint_32 initial_val = 3 * xpng_pass_start[xpng_ptr->pass];
                 // png.c:  xpng_pass_start[] = {0, 4, 0, 2, 0, 1, 0};
               register int stride = 3 * xpng_pass_inc[xpng_ptr->pass];
                 // png.c:  xpng_pass_inc[] = {8, 8, 4, 4, 2, 2, 1};
               register int rep_bytes = 3 * xpng_pass_width[xpng_ptr->pass];
                 // png.c:  xpng_pass_width[] = {8, 4, 4, 2, 2, 1, 1};
               register xpng_uint_32 final_val = 3 * xpng_ptr->width;
...
// preload        "movl      mask, %%edx      \n\t"
            ...
            {
               register xpng_uint_32 i;
               xpng_uint_32 initial_val = 4 * xpng_pass_start[xpng_ptr->pass];
                 // png.c:  xpng_pass_start[] = {0, 4, 0, 2, 0, 1, 0};
               register int stride = 4 * xpng_pass_inc[xpng_ptr->pass];
                 // png.c:  xpng_pass_inc[] = {8, 8, 4, 4, 2, 2, 1};
               register int rep_bytes = 4 * xpng_pass_width[xpng_ptr->pass];
                 // png.c:  xpng_pass_width[] = {8, 4, 4, 2, 2, 1, 1};
               register xpng_uint_32 final_val = 4 * xpng_ptr->width;
...
// preload        "movl      mask, %%edx      \n\t"
            ...
            {
               register xpng_uint_32 i;
               xpng_uint_32 initial_val = 6 * xpng_pass_start[xpng_ptr->pass];
                 // png.c:  xpng_pass_start[] = {0, 4, 0, 2, 0, 1, 0};
               register int stride = 6 * xpng_pass_inc[xpng_ptr->pass];
                 // png.c:  xpng_pass_inc[] = {8, 8, 4, 4, 2, 2, 1};
               register int rep_bytes = 6 * xpng_pass_width[xpng_ptr->pass];
                 // png.c:  xpng_pass_width[] = {8, 4, 4, 2, 2, 1, 1};
               register xpng_uint_32 final_val = 6 * xpng_ptr->width;
         ...
         case 64:       // xpng_ptr->row_info.pixel_depth
         {
            ...
            xpng_bytep dstptr;
            register xpng_uint_32 i;
            xpng_uint_32 initial_val = 8 * xpng_pass_start[xpng_ptr->pass];
              // png.c:  xpng_pass_start[] = {0, 4, 0, 2, 0, 1, 0};
            register int stride = 8 * xpng_pass_inc[xpng_ptr->pass];
              // png.c:  xpng_pass_inc[] = {8, 8, 4, 4, 2, 2, 1};
            register int rep_bytes = 8 * xpng_pass_width[xpng_ptr->pass];
              // png.c:  xpng_pass_width[] = {8, 4, 4, 2, 2, 1, 1};
            register xpng_uint_32 final_val = 8 * xpng_ptr->width;
         ...
         default:   // xpng_ptr->row_info.pixel_depth != 1,2,4,8,16,24,32,48,64
         {
            ...
            fprintf(stderr,
              "libpng internal error:  xpng_ptr->row_info.pixel_depth = %d\n",
...
void
xpng_do_read_interlace(xpng_row_infop row_info, xpng_bytep row, int pass,
   xpng_uint_32 transformations)
{
/*
fprintf(stderr, "GRR DEBUG:  entering xpng_do_read_interlace()\n");
...
/*
fprintf(stderr, "GRR DEBUG:  xpng_do_read_interlace() row_info->width = %d, final_width = %d\n", row_info->width, final_width);
fprintf(stderr, "GRR DEBUG:  xpng_do_read_interlace() pixel_depth = %d\n", row_info->pixel_depth);
fflush(stderr);
 */
...
/*
fprintf(stderr, "GRR DEBUG:  xpng_do_read_interlace() pass = %d, width_mmx = %d, width = %d\n", pass, width_mmx, width);
...
xpng_read_filter_row_mmx_sub(xpng_row_infop row_info, xpng_bytep row)
{
   ...
   switch ( bpp )
   {
      ...
      case 1:
      {
         ...
         // non-MMX code for the SUB filter in xpng_read_filter_row above
         //
         //         xpng_bytep rp;
         //         xpng_bytep lp;
         //         xpng_uint_32 i;
   ...
   row, xpng_bytep prev_row, int filter)
{
...
#endif
   {
       xpng_read_filter_row_c(xpng_ptr, row_info, row, prev_row, filter);
       return ;
   }
   ...
   switch (filter)
   {
      ...
      default:
         xpng_error(xpng_ptr, "Bad adaptive filter type");
         break;
   }
}
...
//      ".byte  0x0f, 0xa2    \n\t"  // CPUID instruction (two-byte opcode)
        ...
        "popl %%edx           \n\t"  // restore edx
        "popl %%ecx           \n\t"  // restore ecx
        "popl %%ebx           \n\t"  // restore ebx ("row" in xpng_do_interlace)
