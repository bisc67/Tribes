...
// Copyright (c) 1995-1999 QSound Labs, Inc.  All Rights Reserved.
// Version 5.00, March 16, 1999.
//  
...
#if defined(QMDX)
    #define QM(name) QMDX_ ## name
#elif defined(QMIXMSS)
    #define QM(name) QMixMSS_ ## name
#else
    #define QM(name) QSWaveMix ## name
#endif


DECLARE_HANDLE(HQMIXER);

#define QMIXER_VERSION 0x0500
...
// - QM(GetChannelParams)
// - QM(SetChannelParams)
// - QM(CalculateChannelParams)
// - QM(FindChannel)
// - QM(PlayEx)
//
// The first set of parameters shouldn't be modified, the second set
// can be altered for QM(SetChannelParams), and the third set
...
struct QMIX_CHANNEL_PARAMS {
    ...
    DWORD dwModes;                  // combination of QM(EnableChannel) flags
    float flReverbMix;              // relative mix level of reverb for this channel
    long unused[15];                // reserved for future expansion (set to zero)
...
#define QMIX_CHANNELTYPE_2D         0x01000000  // stereo mixing (i.e. using pan and volume controls) only
...
// QM(FindChannel) until a sound is played on the channel.  This is
...
//
// Structure describing wave data; returned by QM(OpenWave) and
// QM(OpenWaveEx).
//
...
//
// QM(OpenWave) and QM(OpenWaveEx) flags.
//
...
#define QMIX_FILEHANDLE             0x0005  // load from file handle and offset
...
typedef union {
    ...
    struct {
        ...
        LONGLONG Offset;            // Location in file (64 bits).
        ULONG Length;               // Length of wave.
    } FileHandle;
...
QM(ActivateEx)(
    HQMIXER hQMixer,
    BOOL fActivate,
    HWND hwndMain,                      // new main window (when fActive==TRUE)
    DWORD dwReserved
    );


//
// Options for dwFlags parameter in QM(PlayEx).
//
...
// until the next sound is played with QM(PlayEx).  This allows
...
//   - QM(EnableChannel)
//   - QM(SetChannelParams)
//   - QM(SetChannelPriority)
//   - QM(SetDistanceMapping)
//   - QM(SetFrequency)
//   - QM(SetPan)
//   - QM(SetPanRate)
//   - QM(SetPolarPosition)
...
//   - QM(SetSourceCone)
//   - QM(SetSourceCone2)
//   - QM(SetSourcePosition)
//   - QM(SetSourceVelocity)
//   - QM(SetVolume)
//
...
// out changes.
//
// Use QM(SetPanRate)(hQMixer, iChannel, QMIX_WAIT|QMIX_USEONCE, 0)
// to implement sudden changes for the next sound without losing the
// smoothing function.  Using QMIX_IMMEDIATE with QM(PlayEx) also
...
//
// QM(EnableChannel):  if mode==0, use conventional, high-performance
// stereo mixer.  Non-zero modes imply some form of additional processing.
//
#define QMIX_CHANNEL_STEREO                     0x00000000  // perform stereo mixing
#define QMIX_CHANNEL_QSOUND                     0x00000001  // perform QSound localization (default)
#define QMIX_CHANNEL_DOPPLER                    0x00000002  // calculate velocity using position updates
#define QMIX_CHANNEL_REVERB                     0x00000004  // do reverb effects
#define QMIX_CHANNEL_ELEVATION                  0x00000008  // do elevation effects
#define QMIX_CHANNEL_NODOPPLERPITCH             0x00000010  // disable Doppler pitch shift for this channel
#define QMIX_CHANNEL_PITCH_COPY                 0x00000000  // pitch shifting using copying (fastest)
#define QMIX_CHANNEL_PITCH_LINEAR               0x00000100  // pitch shifting using linear interpolation (better but slower)
#define QMIX_CHANNEL_PITCH_MASK                 0x00000700  // bits reserved for pitch types
#define QMIX_CHANNEL_MUTEATMAXDISTANCE          0x00000000  // mute sound past max distance (default)
#define QMIX_CHANNEL_NOROLLOFFATMAXDISTANCE     0x00000800  // sound at constant volume past max distance
#define QMIX_CHANNEL_LINEARROLLOFF              0x00001000  // linear rolloff with distance
...
//
// QM(Enable) is equivalent to
//   QM(EnableChannel)(hQMixer, 0, QMIX_ALL, mode).
//
_QMIXIMP
MMRESULT
WINAPI
...
enum {
    QMIX_REVERB_TABLE_CREATIVE,
    QMIX_REVERB_TABLE_QSOUND
};

enum {
    QMIX_REVERB_EAX_ENVIRONMENT_GENERIC,                // factory default
    QMIX_REVERB_EAX_ENVIRONMENT_PADDEDCELL,
    QMIX_REVERB_EAX_ENVIRONMENT_ROOM,              // standard environments
    QMIX_REVERB_EAX_ENVIRONMENT_BATHROOM,
    QMIX_REVERB_EAX_ENVIRONMENT_LIVINGROOM,
    QMIX_REVERB_EAX_ENVIRONMENT_STONEROOM,
    QMIX_REVERB_EAX_ENVIRONMENT_AUDITORIUM,
    QMIX_REVERB_EAX_ENVIRONMENT_CONCERTHALL,
    QMIX_REVERB_EAX_ENVIRONMENT_CAVE,
    QMIX_REVERB_EAX_ENVIRONMENT_ARENA,
    QMIX_REVERB_EAX_ENVIRONMENT_HANGAR,
    QMIX_REVERB_EAX_ENVIRONMENT_CARPETEDHALLWAY,
    QMIX_REVERB_EAX_ENVIRONMENT_HALLWAY,
    QMIX_REVERB_EAX_ENVIRONMENT_STONECORRIDOR,
    QMIX_REVERB_EAX_ENVIRONMENT_ALLEY,
    QMIX_REVERB_EAX_ENVIRONMENT_FOREST,
    QMIX_REVERB_EAX_ENVIRONMENT_CITY,
    QMIX_REVERB_EAX_ENVIRONMENT_MOUNTAINS,
    QMIX_REVERB_EAX_ENVIRONMENT_QUARRY,
    QMIX_REVERB_EAX_ENVIRONMENT_PLAIN,
    QMIX_REVERB_EAX_ENVIRONMENT_PARKINGLOT,
    QMIX_REVERB_EAX_ENVIRONMENT_SEWERPIPE,
    QMIX_REVERB_EAX_ENVIRONMENT_UNDERWATER,
    QMIX_REVERB_EAX_ENVIRONMENT_DRUGGED,
    QMIX_REVERB_EAX_ENVIRONMENT_DIZZY,
    QMIX_REVERB_EAX_ENVIRONMENT_PSYCHOTIC,

    QMIX_REVERB_EAX_ENVIRONMENT_COUNT
};

enum {
    QMIX_REVERB_QSOUND_ENVIRONMENT_JEDIKNIGHT,
    QMIX_REVERB_QSOUND_ENVIRONMENT_FORSAKEN,
    QMIX_REVERB_QSOUND_ENVIRONMENT_MOTOCROSSMADNESS,
    QMIX_REVERB_QSOUND_ENVIRONMENT_DEERHUNTER,
    QMIX_REVERB_QSOUND_ENVIRONMENT_DIABLO,
    QMIX_REVERB_QSOUND_ENVIRONMENT_DIEBYTHESWORD,
    QMIX_REVERB_QSOUND_ENVIRONMENT_DUNGEONKEEPER,
    QMIX_REVERB_QSOUND_ENVIRONMENT_FALLOUT,
    QMIX_REVERB_QSOUND_ENVIRONMENT_FALLOUT2,
    QMIX_REVERB_QSOUND_ENVIRONMENT_FIFAWORLDCUP98,
    QMIX_REVERB_QSOUND_ENVIRONMENT_FINALFANTASY7,
    QMIX_REVERB_QSOUND_ENVIRONMENT_HEAVYGEAR,
    QMIX_REVERB_QSOUND_ENVIRONMENT_HEXEN2,
    QMIX_REVERB_QSOUND_ENVIRONMENT_MIGHTMAGICVI,
    QMIX_REVERB_QSOUND_ENVIRONMENT_MYTH,
    QMIX_REVERB_QSOUND_ENVIRONMENT_NEEDFORSPEEDII,
    QMIX_REVERB_QSOUND_ENVIRONMENT_NEVERHOOD,
    QMIX_REVERB_QSOUND_ENVIRONMENT_QUAKE2,
    QMIX_REVERB_QSOUND_ENVIRONMENT_RESIDENTEVIL,
    QMIX_REVERB_QSOUND_ENVIRONMENT_RIVEN,
    QMIX_REVERB_QSOUND_ENVIRONMENT_STARCRAFT,
    QMIX_REVERB_QSOUND_ENVIRONMENT_TESTDRIVE4,
    QMIX_REVERB_QSOUND_ENVIRONMENT_TOMBRAIDER,
    QMIX_REVERB_QSOUND_ENVIRONMENT_TUROK,
    QMIX_REVERB_QSOUND_ENVIRONMENT_XWING_TIEFIGHTER,

    QMIX_REVERB_QSOUND_ENVIRONMENT_COUNT
};
...
MMRESULT
WINAPI
QM(SetReverbEnvironment)(
    HQMIXER hQMixer,
    int iTable,             // REVERB_TABLE_XXX
    int iEnvironment,       // QMIX_REVERB_EAX_ENVIRONMENT_XXX or QMIX_REVERB_QSOUND_ENVIRONMENT_XXX
    DWORD dwFlags
    );
...
MMRESULT
WINAPI
QM(GetReverbDecayTime)(
    HQMIXER hQMixer,
    float* lpfDecayTime_sec
    );

_QMIXIMP
MMRESULT
WINAPI
QM(GetReverbDampingFactor)(
    HQMIXER hQMixer,
    float* lpfDamping
    );
...
//**********************************************************************
// Call QM(Pump) to force more mixing.  Essential if mixer was
...
//
// Note that, in versions 2.10 and later, that QM(FreeWave) can be
// called with a null hQMixer if QM(CloseSession) has been called.
//
_QMIXIMP
MMRESULT
WINAPI
...
//
// Diagnostic codes available from QM(GetLastError).  Equivalent strings
// can be recalled using QM(GetErrorText).
//
...
// when QM(OpenWave) is used with the QMIX_RESIDENT_COMPRESSED
// parameter.
//
...
typedef struct MIXPLAYPARAMS {
    ...
    HQMIXER hMixSession;    // value returned by QM(Init) or QM(ConfigureInit)
    int iChannel;           // channel number
    LPMIXWAVE lpMixWave;    // pointer returned by QM(OpenWave)
    HWND hWndNotify;        // if set, WOM_OPEN and WOM_DONE notification messages sent here
    DWORD dwFlags;
