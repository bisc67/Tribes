...
// Cutoff Default
#define  _FX_CUTOFF_DEFAULT   2000.0
...
static U32 AILEXPORT PROVIDER_query_attribute (HATTRIB index)
{
   switch ((ATTRIB) index)
      {
      case PROVIDER_NAME:      return (U32) "HighPass Filter";
                                           ...
                                           HSAMPLE      S)
{
   ...
   fC    =  (F32)tan( M_PI * SS->fCutoffFreq / (F32)fRate );

   // calculate coefficients
   SS->fA[ 0 ] =  1.0F / ( 1.0F + (F32)sqrt( 2.0L ) * fC + fC * fC );
   SS->fA[ 1 ] =  -2.0F * SS->fA[ 0 ];
   SS->fA[ 2 ] =  SS->fA[ 0 ];

   // calculate coefficients
   SS->fB[ 0 ] =  2.0F * ( fC * fC - 1.0F ) * SS->fA[ 0 ];
   SS->fB[ 1 ] =  ( 1.0F - (F32)sqrt( 2.0F ) * fC + fC * fC ) * SS->fA[ 0 ];
   ...
   switch ((ATTRIB) attribute)
      {
         case  _FX_HIGHPASS_CUTOFF   :
               ...
               return( (S32) float_as_long(SS->fCutoffFreq ));
                                            ...
                                            void FAR*    value)
{
   ...
   switch ((ATTRIB) preference)
      ...
      case  _FX_HIGHPASS_CUTOFF   :
            ...
            prev = (S32) float_as_long(SS->fCutoffFreq );
            SS->fCutoffFreq = *(F32 FAR*)value;

            // get sample rate
            fRate =  SS->driver->rate;

            // clip to valid range
            FX_CLIPRANGE( SS->fCutoffFreq, 20.0F, fRate / 2.0F - 1.0F );
...
extern "C" S32 MSS_RIB_Main( HPROVIDER provider_handle, U32 up_down )
{
   ...
   const RIB_INTERFACE_ENTRY FLTSMP[] =
      ...
      REG_AT("Highpass Cutoff",             _FX_HIGHPASS_CUTOFF,         RIB_FLOAT),
