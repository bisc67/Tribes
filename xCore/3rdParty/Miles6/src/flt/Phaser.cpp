...
static void  FXCalcParams( SAMPLESTATE FAR * SS, HSAMPLE  S )
   {
      ...
      SS->fWP  =  ( M_PI * SS->fBaseFreq ) / fRate;

      // set minimum sweep
      SS->fSweepMin     =  SS->fWP;

      // calculate range
      SS->fSweepRange   =  (F32)pow( 2.0F, SS->fRange );

      // set maximum sweep
      SS->fSweepMax     =  ( M_PI * SS->fBaseFreq * SS->fSweepRange ) / fRate;

      // set sweep rate
      SS->fSweepRate    =  (F32)pow( SS->fSweepRange, SS->fRate / ( fRate / 2.0F ));
...
static U32 AILEXPORT PROVIDER_query_attribute (HATTRIB index)
{
   switch ((ATTRIB) index)
      {
      case PROVIDER_NAME:      return (U32) "Phaser Filter";
                                    ...
                                    U32             mixer_operation)
{
   ...
   if ( mixer_operation & M_DEST_STEREO )
   {
      ...
      for( dwIndex = 0; dwIndex < dwTemp; dwIndex += 2 )
      {
         ...
         fOutput  =  SS->fLY4 * fWetOut + fInput * fDryOut;
         ...
         if ( SS->fWP > SS->fSweepMax )
            SS->fSweepFrac =  1.0F / SS->fSweepRate;
         else
         {
            // check for minimum
   ...
   switch ((ATTRIB) attribute)
      {
         ...
         case  _FX_PHASER_RANGE     :
               ...
               return( (S32) float_as_long(SS->fRange ));

               break;

         case  _FX_PHASER_BASEFREQ  :

               return( (S32) float_as_long(SS->fBaseFreq ));
                                            ...
                                            void FAR*    value)
{
   ...
   switch ((ATTRIB) preference)
      ...
      case  _FX_PHASER_RANGE     :
            ...
            // get previous value and set new
            prev        = (S32) float_as_long(SS->fRange );
            SS->fRange  = *(F32 FAR*)value;
      ...
      case  _FX_PHASER_BASEFREQ  :
            ...
            // get previous value and set new
            prev           = (S32) float_as_long(SS->fBaseFreq );
            SS->fBaseFreq  = *(F32 FAR*)value;

            // clip to valid range
            FX_CLIPRANGE( SS->fBaseFreq, 20.0F, 20000.0F );
...
extern "C" S32 MSS_RIB_Main( HPROVIDER provider_handle, U32 up_down )
{
   ...
   const RIB_INTERFACE_ENTRY FLTSMP[] =
      ...
      REG_AT("Phaser Feedback",             _FX_PHASER_FEEDBACK,         RIB_FLOAT),
      REG_PR("Phaser Feedback",             _FX_PHASER_FEEDBACK,         RIB_FLOAT),
      REG_AT("Phaser Rate",             _FX_PHASER_RATE,         RIB_FLOAT),
      REG_PR("Phaser Rate",             _FX_PHASER_RATE,         RIB_FLOAT),
      REG_AT("Phaser Range",             _FX_PHASER_RANGE,         RIB_FLOAT),
      REG_PR("Phaser Range",             _FX_PHASER_RANGE,         RIB_FLOAT),
      REG_AT("Phaser BaseFreq",             _FX_PHASER_BASEFREQ,         RIB_FLOAT),
      REG_PR("Phaser BaseFreq",             _FX_PHASER_BASEFREQ,         RIB_FLOAT),
      REG_AT("Phaser Mix",             _FX_PHASER_MIX,         RIB_FLOAT),
      REG_PR("Phaser Mix",             _FX_PHASER_MIX,         RIB_FLOAT),
      };
