...
//##  Miles Sound System                                                    ##
//##                                                                        ##
//##  API.CPP: FLT module for Autopan Filter                                ##
//##                                                                        ##
...
// Pan Rate / Mode
#define  _FX_RATE_DEFAULT        2000.0F
#define  _FX_MODE_DEFAULT        1.0F
...
struct SAMPLESTATE
{
   ...
   F32    fPanPosition;        // ( 0.0 left 1.0 center 2.0 right )
   F32    fPanStepValue;
};
...
static void  FXCalcParams( SAMPLESTATE FAR * SS, HSAMPLE  S )
   {
      ...
      // set rate
      SS->fPanStepValue =  ( 2.0F / SS->fRate ) / ( fRate / 1000.0F );
   }
...
static U32 AILEXPORT PROVIDER_query_attribute (HATTRIB index)
{
   switch ((ATTRIB) index)
      {
      case PROVIDER_NAME:      return (U32) "Autopan Filter";
                                           ...
                                           HSAMPLE      S)
{
   ...
   SS->fMode   =  _FX_MODE_DEFAULT;

   // set start pan position
   SS->fPanPosition  =  0.0;
                                    ...
                                    U32             mixer_operation)
{
   ...
   if ( mixer_operation & M_DEST_STEREO )
   {
      ...
      {
         // get pan left / right scale values
         if ( SS->fPanPosition <= 1.0F )
         {
            // set pan scale values
            fPanL =  1.0F;
            fPanR =  SS->fPanPosition;
         }
         else
         {
            // set pan scale values
            fPanL =  1.0F - ( SS->fPanPosition - 1.0F );
            fPanR =  1.0F;
         }

         // adjust pan position
         SS->fPanPosition  += SS->fPanStepValue;

         // check if we have passed either side
         if ( SS->fPanPosition <= 0.0F )
         {
            // reset position
            SS->fPanPosition  =  0.0F;

            // flip direction
            SS->fPanStepValue =  -SS->fPanStepValue;
         }

         // check for right side
         if ( SS->fPanPosition >= 2.0F )
         {
            // reset position
            SS->fPanPosition  =  2.0F;

            // check mode 0.0 == l/r loop, 1.0 = ping-pong
            if ( SS->fMode == 0.0F )
            {
               // reset to left
               SS->fPanPosition  =  0.0F;
         ...
         fInput   =  (F32)dwTempBuild[ dwIndex ] * fPanL;

         // store output
         lpBuildDest[ dwIndex ]  += f2i(fInput);

         // get input sample (right)
         fInput   =  (F32)dwTempBuild[ dwIndex + 1 ] * fPanR;

         // store output
         lpBuildDest[ dwIndex + 1 ]  += f2i(fInput);
      }
   }
   else
   {
                                           ...
                                           HATTRIB      attribute)
{
   ...
   switch ((ATTRIB) attribute)
      {
         case  _FX_AUTOPAN_RATE   :

               return( (S32) float_as_long(SS->fRate ));

               break;

         case  _FX_AUTOPAN_MODE   :

               return( (S32) float_as_long(SS->fMode ));
                                            ...
                                            void FAR*    value)
{
   ...
   switch ((ATTRIB) preference)
      ...
      case  _FX_AUTOPAN_RATE   :
            ...
            prev        = (S32) float_as_long(SS->fRate );
            SS->fRate   = *(F32 FAR*)value;

            // clip frequency to valid range
            FX_CLIPRANGE( SS->fRate, 20.0F, 10000.0F );
      ...
      case  _FX_AUTOPAN_MODE   :
            ...
            // get previous value and set new
            prev = (S32) float_as_long(SS->fMode );
            SS->fMode = *(F32 FAR*)value;

            // clip width to valid range
            FX_CLIPRANGE( SS->fMode, 0.0F, 1.0F );
...
extern "C" S32 MSS_RIB_Main( HPROVIDER provider_handle, U32 up_down )
{
   ...
   const RIB_INTERFACE_ENTRY FLTSMP[] =
      ...
      REG_AT("Autopan Rate",             _FX_AUTOPAN_RATE,         RIB_FLOAT),
      REG_PR("Autopan Rate",             _FX_AUTOPAN_RATE,         RIB_FLOAT),
      REG_AT("Autopan Mode",             _FX_AUTOPAN_MODE,         RIB_FLOAT),
      REG_PR("Autopan Mode",             _FX_AUTOPAN_MODE,         RIB_FLOAT),
      };
