...
// Ringmodulator
#define  _FX_FREQ_DEFAULT        2.0
#define  _FX_MODE_DEFAULT        1.0
#define  _FX_MIX_DEFAULT         0.5
...
static U32 AILEXPORT PROVIDER_query_attribute (HATTRIB index)
{
   switch ((ATTRIB) index)
      {
      case PROVIDER_NAME:      return (U32) "Ring Modulator Filter";
                                    ...
                                    U32             mixer_operation)
{
   ...
   // temp 
   F32    fTempL      =  2.0F * M_PI * SS->fFrequency/SS->fRate;
   ...
   if ( mixer_operation & M_DEST_STEREO )
   {
      ...
      for( dwIndex = 0; dwIndex < dwTemp; dwIndex += 2 )
      {
         ...
         // modulate input
         fOutput   = fInput * (F32)sin( SS->fSampleIndex * fTempL );

         // left
         fOutput  =  ( fInput * fDryOut ) +
                           ( fOutput * fWetOut );
         ...
         if ( SS->fMode == 0.0 )
            ...
            fOutput   = fInput * (F32)cos( SS->fSampleIndex++ * fTempL );
   ...
   switch ((ATTRIB) attribute)
      {
         case  _FX_RINGMOD_FREQ   :
               ...
               return( (S32) float_as_long(SS->fFrequency ));

               break;

         case  _FX_RINGMOD_MODE   :
                                            ...
                                            void FAR*    value)
{
   ...
   switch ((ATTRIB) preference)
      ...
      case  _FX_RINGMOD_FREQ   :
            ...
            prev = (S32) float_as_long(SS->fFrequency );
            SS->fFrequency = *(F32 FAR*)value;

            // get sample rate
            fRate = SS->driver->rate;

            // clip frequency to valid range
            FX_CLIPRANGE( SS->fFrequency, 1.0F, ( fRate / 2.0F ));
...
extern "C" S32 MSS_RIB_Main( HPROVIDER provider_handle, U32 up_down )
{
   ...
   const RIB_INTERFACE_ENTRY FLTSMP[] =
      ...
      REG_AT("Ring Mod Freq",             _FX_RINGMOD_FREQ,         RIB_FLOAT),
      REG_PR("Ring Mod Freq",             _FX_RINGMOD_FREQ,         RIB_FLOAT),
      REG_AT("Ring Mod Mode",             _FX_RINGMOD_MODE,         RIB_FLOAT),
      REG_PR("Ring Mod Mode",             _FX_RINGMOD_MODE,         RIB_FLOAT),
      REG_AT("Ring Mod Mix",             _FX_RINGMOD_MIX,         RIB_FLOAT),
      REG_PR("Ring Mod Mix",             _FX_RINGMOD_MIX,         RIB_FLOAT),
      };
