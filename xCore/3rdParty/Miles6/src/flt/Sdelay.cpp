...
static U32 AILEXPORT PROVIDER_query_attribute (HATTRIB index)
{
   switch ((ATTRIB) index)
      {
      case PROVIDER_NAME:      return (U32) "Stereo Delay Filter";
                                    ...
                                    U32             mixer_operation)
{
   ...
   F32      fDryOut;
   F32      fWetOut;
   U32    dwDelayLengthL =  (U32)(( SS->fDelayL * SS->fRate ) / 1000.0F);
   U32    dwDelayLengthR =  (U32)(( SS->fDelayR * SS->fRate ) / 1000.0F);
   ...
   if ( mixer_operation & M_DEST_STEREO )
   {
      ...
      for( dwIndex = 0; dwIndex < dwTemp; dwIndex += 2 )
      {
         ...
         // set output
         fOutput  =  ( fInput * fDryOut ) + ( SS->pfDelayBufferL[ SS->dwDelayIndexL ] * fWetOut );

         // calculate output sample
         SS->pfDelayBufferL[ SS->dwDelayIndexL ]  =  fInput + (( SS->pfDelayBufferL[ SS->dwDelayIndexL ] * SS->fFeedback ));
         ...
         // set output
         fOutput  =  ( fInput * fDryOut ) + ( SS->pfDelayBufferR[ SS->dwDelayIndexR ] * fWetOut );

         // calculate output sample
         SS->pfDelayBufferR[ SS->dwDelayIndexR ]  =  fInput + (( SS->pfDelayBufferR[ SS->dwDelayIndexR ] * SS->fFeedback ));
   ...
   switch ((ATTRIB) attribute)
      {
         case  _FX_MDELAY_DELAYL :
               ...
               return( (S32) float_as_long(SS->fDelayL ));

               break;

         case  _FX_MDELAY_DELAYR :

               return( (S32) float_as_long(SS->fDelayR ));
                                            ...
                                            void FAR*    value)
{
   ...
   switch ((ATTRIB) preference)
      ...
      case  _FX_MDELAY_DELAYL     :
            ...
            prev           = (S32) float_as_long(SS->fDelayL );
            SS->fDelayL     = *(F32 FAR*)value;

            // clip to valid range
            FX_CLIPRANGE( SS->fDelayL, 0.01F, _FX_MAX_DELAY );
      ...
      case  _FX_MDELAY_DELAYR     :
            ...
            // get previous value and set new
            prev           = (S32) float_as_long(SS->fDelayR );
            SS->fDelayR     = *(F32 FAR*)value;

            // clip to valid range
            FX_CLIPRANGE( SS->fDelayR, 0.01F, _FX_MAX_DELAY );
...
extern "C" S32 MSS_RIB_Main( HPROVIDER provider_handle, U32 up_down )
{
   ...
   const RIB_INTERFACE_ENTRY FLTSMP[] =
      ...
      REG_AT("Stereo Delay TimeL",           _FX_MDELAY_DELAYL  ,         RIB_FLOAT),
      REG_PR("Stereo Delay TimeL",           _FX_MDELAY_DELAYL  ,         RIB_FLOAT),
      REG_AT("Stereo Delay TimeR",           _FX_MDELAY_DELAYR  ,         RIB_FLOAT),
      REG_PR("Stereo Delay TimeR",           _FX_MDELAY_DELAYR  ,         RIB_FLOAT),
      REG_AT("Stereo Delay Feedback",           _FX_MDELAY_FEEDBACK  ,         RIB_FLOAT),
      REG_PR("Stereo Delay Feedback",           _FX_MDELAY_FEEDBACK  ,         RIB_FLOAT),
      REG_AT("Stereo Delay Mix",               _FX_MDELAY_MIX  ,         RIB_FLOAT),
      REG_PR("Stereo Delay Mix",               _FX_MDELAY_MIX  ,         RIB_FLOAT),
      };
