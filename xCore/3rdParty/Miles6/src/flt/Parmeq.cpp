...
static void  FXCalcParams( SAMPLESTATE FAR * SS, HSAMPLE  S )
   {
      ...
      fWn   =  1.0F / ( 2.0F * M_PI * SS->fCenterFreq );
      fWp   =  ( fWn / (F32)tan( fWn / ( 2.0F * fRate )));
...
static U32 AILEXPORT PROVIDER_query_attribute (HATTRIB index)
{
   switch ((ATTRIB) index)
      {
      case PROVIDER_NAME:      return (U32) "Parametric EQ Filter";
                                    ...
                                    U32             mixer_operation)
{
   ...
   if ( mixer_operation & M_DEST_STEREO )
   {
      ...
      for( dwIndex = 0; dwIndex < dwTemp; dwIndex += 2 )
      {
         ...
         SS->fXL   =  1.0F * fInput - SS->fB1 * SS->fXL1 \
                           - SS->fB2 * SS->fXL2;

         // compute second(output) sample
         fOutput        =  SS->fA0 * SS->fXL + SS->fA1 * SS->fXL1 \
                           + SS->fA2 * SS->fXL2;
         ...
         // compute first component of filter
         SS->fXR   =  1.0F * fInput - SS->fB1 * SS->fXR1 \
                           - SS->fB2 * SS->fXR2;

         // compute second(output) sample
         fOutput        =  SS->fA0 * SS->fXR + SS->fA1 * SS->fXR1 \
                           + SS->fA2 * SS->fXR2;
   ...
   switch ((ATTRIB) attribute)
      {
         ...
         case  _FX_PARMEQ_Q :
               ...
               return( (S32) float_as_long(SS->fQ ));

               break;

         case  _FX_PARMEQ_GAIN :

               return( (S32) float_as_long(SS->fGain ));
                                            ...
                                            void FAR*    value)
{
   ...
   switch ((ATTRIB) preference)
      ...
      case  _FX_PARMEQ_GAIN    :
            ...
            // get previous value and set new
            prev        = (S32) float_as_long(SS->fGain );
            SS->fGain   = *(F32 FAR*)value;
...
extern "C" S32 MSS_RIB_Main( HPROVIDER provider_handle, U32 up_down )
{
   ...
   const RIB_INTERFACE_ENTRY FLTSMP[] =
      ...
      REG_AT("ParmEQ Center",             _FX_PARMEQ_CENTER  ,         RIB_FLOAT),
      REG_PR("ParmEQ Center",             _FX_PARMEQ_CENTER  ,         RIB_FLOAT),
      REG_AT("ParmEQ Q",                  _FX_PARMEQ_Q,                RIB_FLOAT),
      REG_PR("ParmEQ Q",                  _FX_PARMEQ_Q,                RIB_FLOAT),
      REG_AT("ParmEQ Gain",               _FX_PARMEQ_GAIN    ,         RIB_FLOAT),
