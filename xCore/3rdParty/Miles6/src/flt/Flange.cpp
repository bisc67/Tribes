...
static U32 AILEXPORT PROVIDER_query_attribute (HATTRIB index)
{
   switch ((ATTRIB) index)
      {
      case PROVIDER_NAME:      return (U32) "Flange Filter";
                                    ...
                                    U32             mixer_operation)
{
   ...
   if ( mixer_operation & M_DEST_STEREO )
   {
      ...
      for( dwIndex = 0; dwIndex < dwTemp; dwIndex += 2 )
      {
         ...
         fOutput  =  ( (F32)SS->pfDelayBuffer[ SS->wEp1 ] * (F32)( SS->dwSweep & 0x0000ffff ) +
                        (F32)SS->pfDelayBuffer[ SS->wEp2 ] * ( SS->fIFrac - (F32)( SS->dwSweep & 0x0000ffff ))) /
                        SS->fIFrac;
...
extern "C" S32 MSS_RIB_Main( HPROVIDER provider_handle, U32 up_down )
{
   ...
   const RIB_INTERFACE_ENTRY FLTSMP[] =
      ...
      REG_AT("Flange Mix",                   _FX_FLANGE_MIX,         RIB_FLOAT),
      REG_PR("Flange Mix",                   _FX_FLANGE_MIX,         RIB_FLOAT),
      REG_AT("Flange Depth",                 _FX_FLANGE_DEPTH,         RIB_FLOAT),
      REG_PR("Flange Depth",                 _FX_FLANGE_DEPTH,         RIB_FLOAT),
      REG_AT("Flange SweepRate",             _FX_FLANGE_RATE,         RIB_FLOAT),
      REG_PR("Flange SweepRate",             _FX_FLANGE_RATE,         RIB_FLOAT),
      REG_AT("Flange Feedback",              _FX_FLANGE_FEEDBACK,         RIB_FLOAT),
