...
static void  FXCalcParams( SAMPLESTATE FAR * SS, HSAMPLE  S )
   {
      ...
      SS->fB2  =  (F32)exp( - ( 2.0F * M_PI ) * ( SS->fBandWidth / fRate ));
      SS->fB1  =  ( -4.0F * SS->fB2 ) / ( 1.0F + SS->fB2 ) * \
                           (F32)cos( 2.0F * M_PI * ( SS->fCenterFreq / fRate ));
      SS->fA0  =  ( 1.0F - SS->fB2 ) * (F32)sqrt( 1.0F - ( SS->fB1 * SS->fB1 ) / ( 4.0F * SS->fB2 ));
   }
...
static U32 AILEXPORT PROVIDER_query_attribute (HATTRIB index)
{
   switch ((ATTRIB) index)
      {
      case PROVIDER_NAME:      return (U32) "Resonator EQ Filter";
                                    ...
                                    U32             mixer_operation)
{
   ...
   if ( mixer_operation & M_DEST_STEREO )
   {
      ...
      for( dwIndex = 0; dwIndex < dwTemp; dwIndex += 2 )
      {
         ...
         // calculate sample
         fOutput  =  SS->fA0 * fInput - SS->fB1 * SS->fYL1 \
                     - SS->fB2 * SS->fYL2;
         ...
         // calculate sample
         fOutput  =  SS->fA0 * fInput - SS->fB1 * SS->fYR1 \
                     - SS->fB2 * SS->fYR2;
   ...
   switch ((ATTRIB) attribute)
      {
         ...
         case  _FX_RESONATOR_WIDTH :
               ...
               return( (S32) float_as_long(SS->fBandWidth ));
                                            ...
                                            void FAR*    value)
{
   ...
   switch ((ATTRIB) preference)
      ...
      case  _FX_RESONATOR_CENTER    :
            ...
            // clip to valid range
            FX_CLIPRANGE( SS->fCenterFreq, 20.0F, ( fRate / 2.0F ) );
      ...
      case  _FX_RESONATOR_WIDTH    :
            ...
            // get previous value and set new
            prev           = (S32) float_as_long(SS->fBandWidth );
            SS->fBandWidth= *(F32 FAR*)value;

            // clip to valid range
            FX_CLIPRANGE( SS->fBandWidth, 20.0F, fRate / 2.0F );
...
extern "C" S32 MSS_RIB_Main( HPROVIDER provider_handle, U32 up_down )
{
   ...
   const RIB_INTERFACE_ENTRY FLTSMP[] =
      ...
      REG_AT("Resonator Center",           _FX_RESONATOR_CENTER  ,         RIB_FLOAT),
      REG_PR("Resonator Center",           _FX_RESONATOR_CENTER  ,         RIB_FLOAT),
      REG_AT("Resonator Width",           _FX_RESONATOR_WIDTH  ,         RIB_FLOAT),
