...
static U32 AILEXPORT PROVIDER_query_attribute (HATTRIB index)
{
   switch ((ATTRIB) index)
      {
      case PROVIDER_NAME:      return (U32) "Compressor Filter";
                                    ...
                                    U32             mixer_operation)
{
   ...
   // compute scale / gain
   fScale   =  ((32768.0F*2048.0F) / (F32)log( 1.0F + SS->fRatio )) ;
   fRatio   =  (( 10.0F - SS->fRatio) / 10.0F) / (32768.0F*2048.0F);
   ...
   if ( mixer_operation & M_DEST_STEREO )
   {
      ...
      for( dwIndex = 0; dwIndex < dwTemp; dwIndex += 2 )
      {
         ...
         if ( fInput < 0.0F )
         {
            fOutput  =  fInput - SS->fGain * ( ( (F32)log( 1.0F - fInput * fRatio ) * fScale ) + fInput );
         }
         else
         {
            fOutput  =  fInput + SS->fGain * ( ( (F32)log( 1.0F + fInput * fRatio ) * fScale ) - fInput );
   ...
   switch ((ATTRIB) attribute)
      {
         case  _FX_COMPRESSOR_RATIO   :
               ...
               return( (S32) float_as_long(SS->fRatio ));
                                            ...
                                            void FAR*    value)
{
   ...
   switch ((ATTRIB) preference)
      ...
      case  _FX_COMPRESSOR_RATIO   :
            ...
            prev        = (S32) float_as_long(SS->fRatio );
            SS->fRatio  = *(F32 FAR*)value;
...
extern "C" S32 MSS_RIB_Main( HPROVIDER provider_handle, U32 up_down )
{
   ...
   const RIB_INTERFACE_ENTRY FLTSMP[] =
      ...
      REG_AT("Compressor Ratio",            _FX_COMPRESSOR_RATIO,         RIB_FLOAT),
      REG_PR("Compressor Ratio",            _FX_COMPRESSOR_RATIO,         RIB_FLOAT),
      REG_AT("Compressor Mix",             _FX_COMPRESSOR_GAIN,         RIB_FLOAT),
