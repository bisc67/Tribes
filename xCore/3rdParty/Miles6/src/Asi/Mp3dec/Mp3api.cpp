...
//       strcpy(ASI_error_text,"End of data reached");
   ...
   *((U32 FAR *)STR->header_buffer) = seek_data;
...
static C8 FAR *       AILEXPORT ASI_error       (void)
{
   if (!AIL_strlen(ASI_error_text))
      {
      return NULL;
      }
...
static ASIRESULT AILEXPORT ASI_startup     (void)
{
   if (ASI_started++)
      {
...
static ASIRESULT      AILEXPORT ASI_shutdown    (void)
{
   if (!ASI_started)
      {
...
static HASISTREAM   AILEXPORT ASI_stream_open (U32           user,  //)
                                        ...
                                        U32           total_size)
{
   ...
   AIL_memset(STR->res,0,sizeof(STR->res));
...
static ASIRESULT      AILEXPORT ASI_stream_close(HASISTREAM stream)
{
...
static S32 AILEXPORT ASI_stream_process (HASISTREAM  stream, //)
                                  void FAR   *buffer,
                                  S32         request_size)
{
   ASISTREAM FAR *STR = (ASISTREAM FAR *) stream;
...
static ASIRESULT AILEXPORT ASI_stream_seek (HASISTREAM stream, //)
                                     S32        stream_offset)
{
   ASISTREAM FAR *STR = (ASISTREAM FAR *) stream;
...
static S32     AILEXPORT ASI_stream_attribute (HASISTREAM stream, //)
                                        HATTRIB    attribute)
{
   ASISTREAM FAR *STR = (ASISTREAM FAR *) stream;
...
static S32 AILEXPORT ASI_stream_set_preference (HASISTREAM stream, //)
                                         HATTRIB    preference,
                                         void FAR*  value)
{
   ASISTREAM FAR *STR = (ASISTREAM FAR *) stream;
   ...
   switch ((ATTRIB) preference)
      ...
      case REQUESTED_RATE:  prev = STR->requested_rate;  STR->requested_rate  = *(S32 FAR*)value; break;
      case REQUESTED_BITS:  prev = STR->requested_bits;  STR->requested_bits  = *(S32 FAR*)value; break;
      case REQUESTED_CHANS: prev = STR->requested_chans; STR->requested_chans = *(S32 FAR*)value; break;
      }
