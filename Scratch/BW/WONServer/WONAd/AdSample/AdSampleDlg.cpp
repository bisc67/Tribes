	...
	m_TestInstall(true)
{
	...
	m_pWonApi = new WONAPICore(true);
}

CAdSampleDlg::~CAdSampleDlg()
{
	delete m_pWonApi;
}
...
BEGIN_MESSAGE_MAP(CAdSampleDlg, CDialog)
	...
	ON_BN_CLICKED(ID_PAUSE_BTN, OnPauseBtn)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CAdSampleDlg::OnInitDialog()
{
	...
	// Initilize add API
	AdApi::AdResult result = m_AdApi.Init("TestProduct", NULL);
...
void CAdSampleDlg::OnPaint() 
{
	...
	else // paint the ad
	{
		if (m_AdViewBMP) // Bitmap
		{
			...
			{
				char* aToolTip;
				result = m_AdApi.GetToolTip(aToolTip);
				if (result == AdApi::AdResult_Success)
				{
					m_ToolTip = aToolTip;
					m_AdApi.ReleaseToolTip(aToolTip);
				}
				HBITMAP hOldBmp = (HBITMAP)SelectObject(hDC, hBmp);			// select Bitmap into memory DC
...
void CAdSampleDlg::OnPauseBtn() 
{
	m_bPaused = !m_bPaused;
	if (m_bPaused)
	{
		GetDlgItem(ID_PAUSE_BTN)->SetWindowText("&Unpause");
	}
	else
	{
		GetDlgItem(ID_PAUSE_BTN)->SetWindowText("&Pause");
	}
	m_AdApi.PauseDownload(m_bPaused);
}
